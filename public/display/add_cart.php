<?php
	...
	if ($_SESSION['login'])
	{
		...
		$i = 0;
		while ($c = mysqli_fetch_array($get_mail)){
			$email = $c['email'];
			$i++;
		}
		$i = 0;
		...
		if ($stock == 0)
		{
			...
			# code...
			echo "<script> alert('To add in your cart you need lo log in first!'); window.open('show_products.php?category=$pro_cat', '_self');</script>";
			return;
	}

<footer style = "position: fixed; bottom: 0; width: 100%; height:250px;
  background: url('http://www.thehospice.in/images/footer.png'); left:0px;">
 <!-- <div style ="float: right; padding: 20px 7px 0px 17px; font-size:20px;color:white;"> <p> Create by: Cornea Alina</p>
  <p> Contact information: <a href="alinacornea18@gmail.com" style="color:white">
  alinacornea18@gmail.com</a> <br/><br/> -->
  <!-- &copy; <?php echo date('Y'); ?> Vintage </p> -->
</div>
</footer>
</body>

</html>
